...
import {IERC4626C} from "src/interface/IERC4626C.sol";

/**
 * @title ERC4626C
 * @dev ERC721 token standard extended with ERC4626-style asset management functionality.
 */
abstract contract ERC4626C is ERC721, IERC4626C {
    ...
    function deposit(
        uint256 assets,
        address receiver
    ) public payable virtual returns (uint256 id) {
    ...
    function _getTotalAssetsByTokenId(
        uint256 id
    ) public view virtual returns (uint256) {
        return _assetsByTokenId[id];
    }
    ...
    function _isTokenHolder(address caller) public view virtual returns (bool) {
