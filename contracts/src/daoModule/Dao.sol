...
contract DAO is Ownable, ReentrancyGuard {
    ...
    constructor(uint256 _quorum) Ownable(_msgSender()) ReentrancyGuard() {
        quorum = _quorum;
    }
    ...
    function vote(
        uint256 _proposalId
    ) public proposalExists(_proposalId) hasNotVoted(_proposalId) {
    ...
    function executeProposal(
        uint256 _proposalId
    ) public onlyOwner proposalExists(_proposalId) nonReentrant {
        Proposal storage proposal = proposals[_proposalId];
        require(
            proposal.status == ProposalStatus.Active,
            "Proposal is not active"
